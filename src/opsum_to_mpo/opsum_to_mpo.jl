...
function build_MPO_with_support!(
  ...
  rightmaps, support::Union{Nothing,Vector{Int64}}
)
  N    = length(H)
  ...
  # Channel indices start at 1 (no identity rail)
  idrow = ll -> 1
  idcol = rl -> 1
  chanr = r  -> r
  chanc = c  -> c
  ...
  function site_tensor(n, ll, rl)
    ...
    for el in tempMPO[n]
      ...
      if A_row == -1 && A_col == -1
        # no backbone → skip scalar-on-scalar
        continue
      elseif A_row == -1
        # entering corridor
        @inbounds for c in 1:size(VR, 2)
          M[idrow(ll), chanc(c)] += ct * VR[A_col, c]
        end
      elseif A_col == -1
        # leaving corridor
        @inbounds for r in 1:size(VL, 2)
          M[chanr(r), idcol(rl)] += ct * conj(VL[A_row, r])
        end
      else
        # corridor → corridor
        @inbounds for r in 1:size(VL, 2), c in 1:size(VR, 2)
          M[chanr(r), chanc(c)] += ct * conj(VL[A_row, r]) * VR[A_col, c]
        end
      end
...
function ctn_svdMPO(
  ...
  crosses_bond(t::Scaled{C,Prod{Op}}, n::Int) where {C} =
    (only(site(t[1])) <= n <= only(site(t[end])))
  ...
  H = MPO(sites)
  build_MPO_with_support!(H, tempMPO, sites, Vs, rightmaps, support)
  return H
end
